...
<body>
    ...
    <h1 class="text-center text-success">Manage Employees</h1><br>
    ...
    <a class="btn btn-primary" href="<?= DOMAIN . "public/index.php?controller=employee&action=add" ?>">Add +</a><br>
    <main>
        <table class="table">
            <thead>
                <tr>
        ...
        <nav aria-label="Page navigation example">
            <ul class="pagination">
                <li class="page-item <?= $currentPage == 1 ? 'disabled' : '' ?>">
                    <a class="page-link" href="<?= DOMAIN . "public/index.php?controller=employee&action=index&page=" . ($currentPage - 1) ?>">Previous</a>
                </li>
                <?php for ($i = 1; $i <= $totalPages; $i++) : ?>
                    <li class="page-item <?= $i == $currentPage ? 'active' : '' ?>">
                    <a class="page-link " href="<?= DOMAIN . "public/index.php?controller=employee&action=index&page=$i" ?>">
                    <?= $i ?></a></li>
                <?php endfor; ?>
                <li class="page-item <?= $currentPage == $totalPages ? 'disabled' : '' ?>">
                    <a class="page-link" href="<?= DOMAIN . "public/index.php?controller=employee&action=index&page=" . ($currentPage + 1) ?>">Next</a>
                </li>
            </ul>
        </nav>
